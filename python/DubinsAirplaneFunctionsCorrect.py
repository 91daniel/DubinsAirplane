...
def computeDubinsRSR(R=None, crs=None, cre=None, anglstart=None,anglend=None):
    ...
    return L # length of dubins path
...
def computeOptimalRadius(zs=None, anglstart=None, ze=None, anglend=None, R_min=None, gamma_max=None, idx=None, k=None, hdist=None):
    ...
    print " computing optimal radius"
    acc = 0.001
    R1 = R_min
    R2 = 2 * R_min
    R = ( R1 + R2 ) / 2
...
def printDubinsPath(dp=None):
    ...
    print "p_s: ", dp['p_s']
    print "p_e: ", dp['p_e']
    print "angl_s: ", dp['angl_s']
    print "angl_e: ", dp['angl_e']
    print "c_s: ", dp['c_s']
    print "c_e: ", dp['c_e']
    print "c_si: ", dp['c_si']
    print "c_ei: ", dp['c_ei']
    print "psi_s: ", dp['psi_s']
    print "psi_e: ", dp['psi_e']
    print "psi_si: ", dp['psi_si']
    print "psi_ei: ", dp['psi_ei'] 
    print "lamda_s: ", dp['lamda_s']
    print "lamda_e: ", dp['lamda_e']
    print "lamda_si: ", dp['lamda_si']
    print "lamda_ei: ", dp['lamda_ei']
    print "k_s: ", dp['k_s']
    print "k_e: ", dp['k_e']
    print "k_ei: ", dp['k_ei']
    print "k_si: ", dp['k_si']
    print "R: ", dp['R']
    print "gamma: ", dp['gamma']
    print "w_s: ", dp['w_s']
    print "q_s: ", dp['q_s']
    print "w_si: ", dp['w_si']
    print "q_si: ", dp['q_si']
...
def DubinsAirplanePath(init_conf=None, final_conf=None, R_min=None, gamma_max=None):
    ...
    if DubinsAirplaneSolution['case'] == 1: # spiral-line-spiral
        if idx == 1: # right-straight-right
            ...
            DubinsAirplaneSolution['w_e']   = ze
            DubinsAirplaneSolution['q_e']   = np.dot(rotz(anglend), np.array([1,0,0]).T)
    elif DubinsAirplaneSolution['case'] == 2: # spiral - spiral - line -spiral
    ...
    elif DubinsAirplaneSolution['case'] == 3: # spiral - line - spiral - spiral
        if idx == 1: # right-straight-left-right
            ...
            theta2 = theta-pi/2 + asin(2*R/ell)
            dist1 = R*fmod(2*pi+fmod(theta2,2*pi) - fmod(anglstart-pi/2,2*pi),2*pi) # distance on start left spiral
            dist2 = R*fmod(2*pi+fmod(theta2+pi/2,2*pi)-fmod(chii+pi/2,2*pi),2*pi) # distance on intermediate left spiral
            dist2 = R*fmod(2*pi+fmod(theta2+pi,2*pi)-fmod(chii+pi/2,2*pi),2*pi)
            dist3 = fabs(R_min*psii) # distance on end right spiral

            w1 = crs + R*np.dot(rotz(theta2),e1.T).T + np.array([0, 0, -dist1*tan(gam)]).T # point where switch from first spiral to line (z coord: start + length*gam*(1))
            w2 = ci + R*np.dot(rotz(theta2+pi),e1.T).T - np.array([0, 0, -(dist2+dist3)*tan(gam)]).T # point where switch from line to second spiral (z coord: end - length*gam*(1))
            q1 = (w2-w1)/np.linalg.norm(w2-w1,ord=2) # direction of line
            # start spiral
            DubinsAirplaneSolution['c_s']   = crs
            DubinsAirplaneSolution['psi_s'] = anglstart-pi/2 # angle on circle to starting position of plane
            DubinsAirplaneSolution['lamda_s'] = 1 # direction of rotation (1=cw)
            DubinsAirplaneSolution['k_s']   = 0 # nbr of full circles
            # intermediate-end spiral
            DubinsAirplaneSolution['c_ei'] = ci - np.array([0, 0, -(dist2+dist3)*tan(gam)]).T # center of intermediate spiral (z coord: end-length*gam*(-1duetoCOSpointingdown))
            DubinsAirplaneSolution['psi_ei'] = theta2+pi # angle to point on intermediate spiral where plane arrives on spiral
            DubinsAirplaneSolution['lamda_ei'] = -1 # direction of rotation (-1=ccw)
            ...
            DubinsAirplaneSolution['k_e'] = 0
            # hyperplane H_s: switch from first to second spiral
            DubinsAirplaneSolution['w_s'] = w1 # point
            DubinsAirplaneSolution['q_s'] = q1 # direction
            # hyperplane H_l: switch from straight-line to intermediate spiral
            DubinsAirplaneSolution['w_l'] = w2 # point
            DubinsAirplaneSolution['q_l'] = q1 # direction
            # hyperplane H_ei: switch from intermediate spiral to
            # end spiral
            DubinsAirplaneSolution['w_ei']  = zi - np.array([0, 0, -dist3*tan(gam)]).T # point where switch from intermediate spiral to end spiral
            DubinsAirplaneSolution['q_ei']  = np.array([cos(chii), sin(chii), 0]).T # direction where switch from intermediate spiral to end spiral
            # hyperplane H_e: end of Dubins path
            DubinsAirplaneSolution['w_e'] = ze # end point
            DubinsAirplaneSolution['q_e'] = np.dot(rotz(anglend),np.array([1,0,0]).T) # end heading of airplane
    ...
    #printDubinsPath(DubinsAirplaneSolution)
...
def TestDrawingSpiral(DubinsAirplaneSolutions=None):
    # Extract the Dubins Airplane Solution in vector form
