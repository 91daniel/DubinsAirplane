...
def main():
    ...
    t0 = time.clock()
    VehiclePars = VehicleParameters( 15, pi/4, 0.5 ) 
    ...
    if flag_nc == 0:
        ...
        SpiralEnd = DubinsAirplanePath( start_node, end_node, R_min, VehiclePars.Gamma_max )
        if ExFlags.verbose :
            PrintSolutionAgainstMATLAB( DubinsAirplaneSolution )
        #DubinsAirplaneSolution['psi_e'] = 4.96517118301+1.55*pi
        path_dubins_airplane = ExtractDubinsAirplanePath( DubinsAirplaneSolution )
        path_dubins_airplane = path_dubins_airplane.T
        SpiralEnd['R'] = 1
        SpiralEnd['gamma']=-0.5
        SpiralEnd['c_e']= np.array([ 2.64278761,5.23395556,4.28751462])
        SpiralEnd['psi_e']=4.96517118301+1.55*pi
        SpiralEnd['lamda_e']=1
        SpiralEnd['k_e']=0
        SpiralEnd['w_e']= np.array([2,6,7])
        SpiralEnd['c_ei']= np.array([ 0.8378673,4.37240889,2.61254941])
        SpiralEnd['psi_ei']=3.51135274401
        SpiralEnd['lamda_ei']=-1
        SpiralEnd['k_ei']=0
        SpiralEnd['w_ei']= np.array([1.74032744,4.80318224,4.28751462])
        SpiralEnd['q_ei']= np.array([0.43077331,-0.90246017,0.])
        print DubinsAirplaneSolution['psi_ei']
        print DubinsAirplaneSolution['psi_e']

        path_dubins_airplane1 = TestDrawingSpiral( SpiralEnd )
